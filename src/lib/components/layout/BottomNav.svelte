...
<div class="fixed bottom-0 left-0 z-40 w-full border-t border-border/40 bg-background/95 backdrop-blur-lg md:hidden">
	<nav class="container mx-auto flex h-16 max-w-lg items-center justify-around px-2">
		{#each navItems as item}
			...
			<a
				href={item.href}
				class="flex flex-col items-center justify-center gap-1 px-3 py-1 text-xs font-medium transition-all {isActive ? 'text-primary' : 'text-muted-foreground hover:text-foreground'}"
			>
